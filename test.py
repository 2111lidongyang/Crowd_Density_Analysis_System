...
class VideoCamera(object):
    def __init__(self, _camera=False):
        ...
        self.max_frame = 160
        if self._camera:
            # self.video = cv2.VideoCapture(0)  # 提供检测视频或者调用摄像头
...
#
# cap = VideoCamera(_camera=True)
# while True:
#     result = cap.get_frame()  # 获取图片字节流和人数
#     if result is not None:
#         frame_bytes, _, _, person_count = result
#         # 继续处理 frame_bytes 和 person_count
...
# def video_feed():
#     # e.set()
#     return Response(gen(VideoCamera(_video=True, video_path=1)),
